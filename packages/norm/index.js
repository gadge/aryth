...
const FRA_SCA = -4294967296; // N2P32: -2^32, scale xorshift() output to produce num ∈ [0,1].
...
class Norm {
  ...
  init() {
    ...
    for (let i = 126; i >= 1; i--) {
      curr = sqrt(-2 * log(BASE_VOL / curr + exp(-0.5 * curr * curr)));
  ...
  retail(hz, iz) {
    ...
    while (true) {
      ...
      if (iz === 0) {
        ...
        return hz > 0 ? TAIL_INI + x : -3.442619855899 - x
      }
...
const n = Walk.next.bind(Walk);

export { Norm, Walk, n, normIter };
